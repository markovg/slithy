...
#             j += 1
    ...
    def toggle_fullscreen( self ):
        if self.fullscreen:
            ...
            self.root._root().geometry("%dx%d+0+0" % self.screen_size)
            self.root._root().wm_geometry("%dx%d+0+0" % self.screen_size)
            #self.fodder.destroy()
            self.root.tkraise()
            self.s.focus()
        else:
            ...
            if sys.platform == 'linux2':
                ...
                self.root.geometry("%dx%d+50+50" % (400, 300))
                #self.root._root().wm_geometry("%dx%d+0+0" % (w, h)) 
                self.root.wm_overrideredirect( 1 )
                self.root.update_idletasks()
                #self.root.grab_set_global()

                self.root._root().tkraise()
                self.root._root().focus_force()
                #self.s.focus()
                #self.root.update()
...
#              return
        ...
        elif keycode in self.key_dispatch:
            self.key_dispatch[keycode](self)
        else:
            try:
                t = self.pdict['t']
