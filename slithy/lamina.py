...
class LaminaPanelSurface(object): 
    ...
    def __init__(self, quadDims=(-1.0,-1.0,2.0,2.0), winSize=None):
        ...
        left, bottom, width, height = quadDims
        right, top = left+width, bottom+height
        self._qdims = quadDims
        self.dims = (left,bottom,0.0), (right,bottom,0.0), (right,top,0.0), (left,top,0.0) 
        self.alpha = None
        self.clear()
    ...
    def clear(self):
        ...
        self._surfTotal = pygame.Surface((powerOfTwo, powerOfTwo), pygame.SRCALPHA, 32)
    ...
    def refresh(self, dirty=None):
        ...
        """
        if not self._txtr:
            self._txtr = load_texture(self._surfTotal,alpha = self.alpha)
        else:
    ...
    def testMode(self,alpha=255):
        """Draw red/transparent checkerboard. """
        w, h = self._winSize[0]*0.25, self._winSize[1]*0.25
        Rect = pygame.Rect
        pygame.draw.rect(self.surf, (250,0,0,alpha), Rect(0,0,w,h), 0)
        pygame.draw.rect(self.surf, (250,0,0,alpha), Rect(2*w,0,w,h), 0)
        
        pygame.draw.rect(self.surf, (250,0,0,alpha), Rect(w,h,w,h), 0)
        pygame.draw.rect(self.surf, (250,0,0,alpha), Rect(3*w,h,w,h), 0)
        
        pygame.draw.rect(self.surf, (250,0,0,alpha), Rect(0,2*h,w,h), 0)
        pygame.draw.rect(self.surf, (250,0,0,alpha), Rect(2*w,2*h,w,h), 0)
        
        pygame.draw.rect(self.surf, (250,0,0,alpha), Rect(w,3*h,w,h), 0)
        pygame.draw.rect(self.surf, (250,250,0,alpha), Rect(3*w,3*h,w,h), 0)
        #self.clear = None
